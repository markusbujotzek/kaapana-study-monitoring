...
class LocalGetMetricsOperator(KaapanaPythonBaseOperator):
    ...
    def start(self, ds, **kwargs):
        ...
        print("")
        print(f"# {self.metrics_endpoint=}")
        print(f"# {self.component_id=}")
        print(f"# {self.timeout=}")
        print(f"# {self.verify_ssl=}")
        print("")
    ...
    def __init__(
        self, dag, metrics_endpoint, component_id, timeout=15, verify_ssl=False, **kwargs
    ):
